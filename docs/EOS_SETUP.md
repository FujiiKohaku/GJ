...
- 定員は3人。1～3人の参加者全員が「準備完了」にすると、ホストが選択ステージを開始できます。ステージ変更はホストが行い、参加者へ同期します。
- EOS ConnectのDevice IDで端末ごとにログインし、EOS LobbyとEOS P2Pを使用します。ゲーム内でEpicアカウント入力やロビーコード入力はありません。
- ホストが毎秒30回、参加中の1～3人の入力をまとめてReliable Orderedで配信します。各端末は同じ固定時間で地形・罠・スイッチ・復活・足場を処理します。ステージファイルのハッシュと各フレームの状態ハッシュを照合し、不一致は画面に表示して終了します。
- オンライン用シーンは `OnlineGamePlayScene`。参加人数に応じた1～3体の流体スライムと個別カメラ、共有チェックポイント、誰か1人のゴールで全員クリアです。形状作成中は本編と同じ速度まで全員の世界がスローになり、スライムの目が×印になります。硬化死亡では本編と同じ粒子形状と×目の死体を残し、炎の核が1.5秒かけて復活地点へ移動してから再生成します。落下死では死体を残しません。足場の当たり判定は確定した同期形状から生成します。
- ホスト移譲・途中参加・途中復帰は行いません。切断や残機切れの際はロビーを作り直します。複数台で遊ぶ場合はすべて同じGitリビジョンを使用してください。異なるビルド間の決定性やインターネット上の実測遅延は未検証です。
...
1. 同じ構成でビルドしたexe、DLL、resourcesを1～3台へ配置します。SDK有効ビルドが成功したことを確認します。
2. 各PCでタイトルをクリックしてステージセレクトへ進み、「オンラインに接続」をクリックします。
3. 1台で「ロビーを作る」、残る1～2台で「参加 / 一覧を更新」→一覧の「参加」をクリックします。ロビー名・コードの入力は不要です。
4. 全端末で1～3人の表示を確認します。ホストが前後のステージボタンをクリックし、参加者側にも選択が反映されることを確認します。
5. 参加者全員が「準備完了」をクリックします。ホストが表示された人数で「このステージを開始」をクリックします。
...
通信データの検証テストは `Tools/Tests/OnlineProtocolTests.vcxproj` をRelease/x64でビルドし、`generated/eos-tests/OnlineProtocolTests.exe` を実行します。1～3人分のCBOR変換、入力トリガーの保持・消費、旧マッチID、不正パケット、範囲外入力、NaN/Infinityを検証します。

EOS SDKとPortalの設定が未配置の環境では、実サービスへのログイン、ロビー検索・参加、複数端末間通信を検証できません。SDK有効ビルドと上記の1～3台試験は、設定後に必ず実施してください。
