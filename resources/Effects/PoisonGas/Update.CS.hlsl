...
void main(uint32_t3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    {
        float32_t3 noise = MakeNoise(particleIndex, gPerFrame.time * 0.55f);
        noise.y *= 0.35f;
        particle.velocity +=
            noise * gEffectSettings.noiseStrength * deltaTime;
    }
    ...
    float expansion = smoothstep(0.0f, 1.0f, progress);
    float scale = lerp(
        gEffectSettings.startScale,
        gEffectSettings.endScale,
        expansion);
    scale *= max(particle.padding.x, 0.01f);
    particle.scale = float32_t3(scale * 1.3333333f, scale, scale);

    particle.color = lerp(
        gEffectSettings.startColor,
        gEffectSettings.endColor,
        progress);
    float fadeIn = smoothstep(0.0f, 0.10f, progress);
