...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    {
        float32_t3 noiseVel = MakeNoise(particleIndex, gPerFrame.time * 2.3f);
    ...
    float flicker = 0.85f + sin(gPerFrame.time * 36.0f + (float)particleIndex * 2.1f) * 0.15f;
    float4 color = lerp(gEffectSettings.startColor, gEffectSettings.endColor, lifeRate);
    color.rgb *= flicker;

    // 重力落下中にアルファ値を減算してハラハラ消滅
