...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    float expandRate = 1.0f - pow(1.0f - lifeRate, 2.2f);
    float lifeVariation = saturate(gParticles[particleIndex].lifeTime / max(gEffectSettings.lifeTime, 0.01f));
    float scale = lerp(gEffectSettings.startScale, gEffectSettings.endScale, expandRate);
    scale *= lerp(0.75f, 1.35f, lifeVariation);
    gParticles[particleIndex].scale = float32_t3(scale, scale, scale);

    float4 yellowEdge = float4(1.0f, 0.76f, 0.18f, 0.95f);
    float4 orangeEdge = float4(1.0f, 0.24f, 0.02f, 0.75f);
    float4 darkEdge = float4(0.16f, 0.035f, 0.0f, 0.0f);
    float4 color;

    if (lifeRate < 0.35f)
    {
        color = lerp(yellowEdge, orangeEdge, lifeRate / 0.35f);
    }
    else
    {
        color = lerp(orangeEdge, darkEdge, (lifeRate - 0.35f) / 0.65f);
    }
