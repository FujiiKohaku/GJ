...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    {
        gParticles[particleIndex].velocity.y += gEffectSettings.gravity * gPerFrame.deltaTime;
    }
    ...
    {
        gParticles[particleIndex].velocity += MakeNoise(particleIndex, gPerFrame.time) * gEffectSettings.noiseStrength * gPerFrame.deltaTime;
    }

    if (gEffectSettings.enableAttraction != 0)
    {
        float32_t3 toEmitter = gEmitter.translate - gParticles[particleIndex].translate;
        float distanceToEmitter = length(toEmitter);
        if (distanceToEmitter > 0.0001f)
        {
            gParticles[particleIndex].velocity += toEmitter / distanceToEmitter * gEffectSettings.attractionStrength * gPerFrame.deltaTime;
        }
    }
