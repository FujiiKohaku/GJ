...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    {
        float32_t3 noiseVel = MakeNoise(particleIndex, gPerFrame.time * 2.2f);
    ...
    float flicker = 0.85f + sin(gPerFrame.time * 30.0f + (float)particleIndex * 2.3f) * 0.15f;
    float4 color = lerp(gEffectSettings.startColor, gEffectSettings.endColor, lifeRate);
    color.rgb *= flicker;

    // 重力落下中にアルファ値をスムーズに減算・消滅
    float alphaFade = 1.0f - pow(lifeRate, 1.4f);
    color.a *= alphaFade;
