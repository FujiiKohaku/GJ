...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    float32_t3 rndSeed = float32_t3(
        (float)DTid.x * 31.71f + gPerFrame.time * 8.9f,
        (float)DTid.x * 61.19f - gPerFrame.time * 3.7f,
        (float)DTid.x * 97.43f + gPerFrame.time * 5.3f
    );
    ...
    float speed = lerp(7.0f, 17.0f, rnd.z);
    ...
    gParticles[particleIndex].translate = gEmitter.translate + (rnd - 0.5f) * 0.3f;
    ...
    // 重厚なロイヤルブルー〜サファイアブルーの深みカラー
    float32_t4 deepBlue = lerp(
        float32_t4(0.05f, 0.3f, 1.0f, 1.0f),
        float32_t4(0.0f, 0.08f, 0.75f, 1.0f),
        rnd.x
    );
    gParticles[particleIndex].color = deepBlue;
