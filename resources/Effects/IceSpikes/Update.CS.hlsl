...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    // 突き出て拡大 ＋ スムーズフェードアウト
    float currentScale = lerp(gEffectSettings.startScale, gEffectSettings.endScale, pow(lifeRate, 0.5f));
    gParticles[particleIndex].scale = float32_t3(currentScale, currentScale * 2.2f, currentScale);

    float4 color = lerp(gEffectSettings.startColor, gEffectSettings.endColor, lifeRate);
    color.a *= (1.0f - pow(lifeRate, 1.5f));
