...
{
    float32_t base = (float32_t) index + salt + gPerFrame.time * 97.13f;
    return float32_t3(
        Hash(base * 12.9898f),
        Hash(base * 78.233f),
        Hash(base * 37.719f));
...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    float32_t3 random = Random3(DTid.x, 11.7f);
    float32_t3 direction = RandomUnitVector(random);

    float startRadius = gEmitter.radius * Hash((float32_t) DTid.x * 5.37f + 0.31f);
    float speed = 11.0f + Hash((float32_t) DTid.x * 9.17f + 2.41f) * 15.0f;
    float scale = gEffectSettings.startScale * (0.7f + Hash((float32_t) DTid.x * 3.91f + 6.2f) * 0.75f);
    float lifeTime = gEffectSettings.lifeTime * (0.75f + Hash((float32_t) DTid.x * 4.73f + 8.9f) * 0.45f);

    gParticles[particleIndex].translate =
        gEmitter.translate + direction * startRadius;
