...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    {
        float32_t3 noiseVel = MakeNoise(particleIndex, gPerFrame.time * 1.8f);
        gParticles[particleIndex].velocity += float32_t3(noiseVel.x * 2.2f, abs(noiseVel.y) * 2.0f + 0.5f, noiseVel.z * 2.2f) * gEffectSettings.noiseStrength * gPerFrame.deltaTime;
    }
    ...
    // 点滅チラツキ
    float flicker = 0.8f + sin(gPerFrame.time * 25.0f + (float)particleIndex * 1.3f) * 0.2f;
