...
int main() {
    ...
    frame["inputs"] = nlohmann::json::array({EncodeInput(decoded), EncodeInput(decoded), EncodeInput(decoded)});
    const auto bytes = Encode(frame);
    Check(bytes.size() <= MaxPacket && Decode(bytes, "match-a") == frame, "Three-player frame roundtrip failed");
    ...
    std::cout << "Online protocol tests passed (roundtrip, 3 players, input accumulation, stale/malformed/out-of-range packets).\n";
}
